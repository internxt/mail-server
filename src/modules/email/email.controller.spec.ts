...
import { EmailController, STUB_USER } from './email.controller.js';
import { EmailService } from './email.service.js';
import { newMailbox, newEmailSummary } from '../../../test/fixtures.js';
...
describe('EmailController', () => {
  ...
  describe('getMailboxes', () => {
    it('When getMailboxes is called, then it returns the mailboxes', async () => {
      ...
      const result = await controller.getMailboxes();
  ...
  describe('list', () => {
    it('When list is called with no query params, then it uses defaults', async () => {
      ...
      const result = await controller.list('inbox');

      expect(emailService.listEmails.mock.calls[0]).toEqual([
        STUB_USER,
        'inbox',
        20,
        0,
    ...
    it('When list is called with limit and position, then it parses them', async () => {
      ...
      await controller.list('sent', '10', '5');
    ...
    it('When list is called with non-numeric strings, then it falls back to defaults', async () => {
      ...
      await controller.list('inbox', 'abc', 'xyz');
